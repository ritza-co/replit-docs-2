...
To display 3D objects on the screen we need three things: a scene, camera and renderer. Then we will place 3D objects in the scene. Most objects also require lighting to be added to see them. 
...
1. Geometry - An object that contains the x, y, z points that makeup a shape.

2. Material - Surface of the geometry. Gives the geometry color and texture.
...
Before we get our player box moving, let's change our code so that is nicely structured for the game logic that we will add. Add the following lines to the top of the `script.js` file:
...
Now let's add some obstacles for the player box to avoid to make it a game. Set the `numOfObstacles` global variable to 50. We are going to create a `createObstacle` function to create 50 randomly positioned obstacles. Add the following function below the `createBox` function:
...
Try to collide with an obstacle, you will see that the alert message is always "You win!". Let's fix that by adding some win or lose logic. In the `detectCollisions` function replace the for loop with the following for loop:
...
There is one big problem with the game, once the game is over we need to refresh the page to play again. That's not a very good user experience. Lets add a play button that will be shown on page load and when the game is over. It will start the game and reset everything that needs to be reset at the start of a game.
