...
If you prefer JavaScript, you can find [a translation of this tutorial using NodeJS](https://www.codementor.io/@garethdwyer/building-a-discord-bot-with-node-js-and-repl-it-mm46r1u8y) instead of Python.
...
### Creating a bot in Discord and getting a token
 You can sign up for a free account over at [the Discord register page](https://discord.com/register), and can download one of their desktop or mobile applications from [the Discord homepage](https://discord.com). You can also use Discord in the browser.
...
In `main.py` we need to start up the web server just before you start up the bot. Add this line of code to `main.py`, just before the line with `token = os.environ.get("DISCORD_BOT_SECRET")`:
...
In the next [tutorial](https://www.codementor.io/@garethdwyer/building-a-discord-bot-with-node-js-and-repl-it-mm46r1u8y), we'll build exactly the same bot again but using NodeJS instead of Python. Even if you prefer Python, it's often a good idea to build the same project in two languages so that you can better appreciate the differences and similarities.
