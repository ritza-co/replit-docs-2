...
Now download [these card images](link) to your computer. Unzip the file, and drag and drop the images it container to the `images` folder in your repl. 
...
**Line 1** of the this code starts up the pygame system, by [initializing](https://www.pygame.org/docs/ref/pygame.html?highlight=init#pygame.init) its modules (for example, the font, sound and graphics code).
...
Python is an object oriented language. With object programming, we identify different parts and entities of the game and model them as classes and objects. Let's draw the game, and see what parts we need to model.

![game-objects](/images/author/game-objects.png)
...
![class-model](/images/author/class-model.png)
...
Alright, the boiler plate is out of the way. Now, lets define the Card class.
...
Every deck of cards needs to be shuffled before it is used, so we've defined a `shuffle`. Python's [random](https://docs.python.org/3/library/random.html) module has a very handy `shuffle` function built in that we can use. `shuffle` take a list and re-arranges the contents of that list in place (i.e, it doesn't return a new list). 
...
We'll use the  `add` method is used a player plays a card, i.e. adds it to the pile. 

The `peek` method returns the top card of the pile, while still keeping it on the pile. We'll use this to draw the top card, as it is face-up. If there are no cards on the pile, indicated by the `len`gth of the card array being 0, we'll return `None`.
...
If a player calls Snap!, we need to check if the top two cards are the same value. We'll use the `isSnap` method to check this. Python has a handy feature of negative indices. This means the an index of `-1` returns the last element in the list, `-2` returns the second last, and so on. This allows us to easily get the last 2 cards added to the pipe, and check if they are the same value. Note that in the rules of snap, only the value is important - the suite is not used. 
...
Besides the `enum` and `pygame` libraries, we import everything, `*`, from our `models`.py file. This way we can use all the classes  we defined in our models.
...
In the properties, we define the `deck` of cards, `player`s, `pile` of cards, and the `state` of the game. We also have a `currentPlayer` property, which keeps track of the player whose turn it is. The `result` property is used to communicate the outcome of a round, or the entire game. 
...
The deal method is in charge of making sure each player gets half the deck of cards. There are a few ways to do this - this code takes a bit of a literal translation of dealing, by dealing one card to each player alternately. In practice, we'll use a for loop to do this, and in the we'll call the `draw` method on each of the `player`s to draw a card.

Since each round of the `for` loop take a card for each player, 2 cards per loop, we only need to loop for half the number of cards in the deck.
...
Here we check which the current player is, and switch `currentPlayer` to the other player.
...
Now let's add some of the logic. The first thing is to check if the current player has pressed the key to play, or `flip` a card onto the pile. If they have pressed their `flipKey`, we add call their play method and add the returned card to the pile. Then we switch turn to the next player, by calling our `switchPlayer` method.
...
Open up the `main.py` file again. We'll start by adding references to the model and engine, so that we can access them when building the UI Add the following imports right under the `import pygame` file.
...
We start off by defining a variable `run`. Then we use this as a condition for a `while` loop. As long as `run` is true, the loop will continously run the code inside.
...
Inside the `renderGame` function, we first clear the window. We use the `fill` method to fill the window with a color, `(15,0,169)` in RGB (Red Green Blue) color notation, which is a dark blue.
...
- **Playing.** The game is in progress, and we are waiting for the current player to flip a card, or either player to call snap. In this case, we render the current player's name and a message to indicate that it is their turn to flip. This message is writtten in white (255,255,255) near the top left of the window. 
...
![playing snaPy](/images/author/game.gif)
